...
export default function HomeScreen() {
  ...
  // Toma el nombre del usuario registrado o 'Usuario' por defecto
  ...
  const loadPetCount = useCallback(async () => {
    try {
      ...
      try {
        const q = query(
          mascotasRef,
          or(where('userId', '==', currentUser.uid), where('ownerId', '==', currentUser.uid))
        );
        const snapshot = await getDocs(q);
        setPetCount(snapshot.size);
      } catch {
        const q1 = query(mascotasRef, where('userId', '==', currentUser.uid));
        const q2 = query(mascotasRef, where('ownerId', '==', currentUser.uid));
        const [snap1, snap2] = await Promise.all([
          getDocs(q1).catch(() => null),
          getDocs(q2).catch(() => null),
        ]);
        const ids = new Set<string>();
        snap1?.docs.forEach((d) => ids.add(d.id));
        snap2?.docs.forEach((d) => ids.add(d.id));
        setPetCount(ids.size);
      }
    } catch {
      setPetCount(0);
    }
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      >
        ...
        >
          <View style={styles.cardHeader}>
            ...
            <View style={styles.cardBadge}>
              <Text style={styles.cardNumero}>{petCount}</Text>
            </View>
          </View>

          <Text style={styles.cardTitulo}>Mis mascotas</Text>
          <Text style={styles.cardDetalle}>
            {petCount === 1 ? '1 mascota registrada' : `${petCount} mascotas registradas`}
          </Text>
        ...
        {/* Grilla de 6 botones (incluyendo QR y Mapa) */}
        <View style={styles.grid}>
          <BotonMenu
            titulo="Escanear QR"
            subtitulo="Identificar o vincular"
          ...
          <BotonMenu
            titulo="Mapa"
            subtitulo="Ubicación y clínicas"
            icono="map-outline"
